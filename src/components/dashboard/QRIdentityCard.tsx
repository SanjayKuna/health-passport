    ...
    <Card variant="elevated">
      <CardContent className="p-6">
        <h3 className="text-lg font-bold text-foreground flex items-center gap-2 mb-6">
          <QrCode className="w-5 h-5 text-primary" /> Your Health QR
        </h3>
        <div className="flex flex-col items-center">
          <ScannableQRCode value={userQRUrl} size={192} className="mb-6" />
          <p className="text-sm text-muted-foreground text-center mb-4">Doctors can scan this code to view your health summary</p>
          <div className="flex gap-2 w-full">
            <Button variant="outline" className="flex-1" onClick={handleCopy}><Copy className="w-4 h-4 mr-2" />Copy Link</Button>
            <Button variant="default" className="flex-1" onClick={handleShare}><Share2 className="w-4 h-4 mr-2" />Share</Button>
          </div>
        </div>
      </CardContent>
    </Card>
